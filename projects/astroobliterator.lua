...
if SERVER then
    ...
    hook.add("tick", "WarpTimer", function()
        if Warp == 1 or val6 > 0.5 then
            val6 = val6 + game.getTickInterval()
        end
        
        if Warp == 2 and val6 < 100000 then
            val6 = 100000
        end
        
        if val6 > 100001.2 and val6 < 100001.4 then
            Warp = 0
        end
        ...
        if Warp == 1 and isValid(astro.driver) and isValid(ghost_camera_prop) then
        ...
        if Warp == 2 and val6 > 100000 and val6 < 100000.045 then
        ...
        if Warp == 1 and val6 > 0.7 and val6 < 100000 then
            body.base[1]:setPos(math.lerpVector(0.3, body.base[1]:getPos(), Vector(50000,50000,50000)))
        end
        
        if Warp == 2 and val6 > 100000 and val6 < 100000.1 then
            body.base[1]:setParent(body_hitbox)
        end
        
        if Warp == 2 and val6 > 100000.1 and val6 < 100001 then
            seat:setParent(body_hitbox)
    ...
    hook.add("InputPressed", "Controls", function(ply, key)
        ...
        if Warp == 1 then
            if key == KEY.F and can_warp and val6 > 5 then
                Warp = 2
    ...
    local function createHud(camerapoint, body)
        ...
        hook.add("DrawHUD", "", function()
            ...
            if camode == 1 then
                ...
                -- Debug info
                if isValid(ghost_camera_entity) then
                    local pos = ghost_camera_entity:getPos()
                    local posStr = string.format("Cam: %.0f, %.0f, %.0f", pos.x, pos.y, pos.z)
                    render.drawText(sw / 2, sh * 0.15, posStr, TEXT_ALIGN.CENTER)
                end
            end
        end)
